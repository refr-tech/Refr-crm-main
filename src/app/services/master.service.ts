...
export class MasterService {
  ...
  constructor(
    ...
    getBurnCatByID(id:string){
      const catData = doc(this.firestore, `${this.auth.resource.env.db.burnCats}`, `${id}`)
      return docData( catData );
    }

    getBurnCatsList(){
      const catDataC = collection(this.firestore, `${this.auth.resource.env.db.burnCats}`)
      const qu = query(catDataC, orderBy("tit", "desc"));
      return collectionData( qu );
    }

    getMyStore(uid: string){
      const catDataC = collection(this.firestore, `${this.auth.resource.env.db.shops}`)
      const qu = query(catDataC, where("by", "==", uid));
      return collectionData( qu );
    }

    getStoreByID(storeID:string){
      const shopRef = doc(this.firestore, `${this.auth.resource.env.db.shops}`, `${storeID}`)
      return getDoc(shopRef)
    }

    getUserByUID(UID:string){
      const userRef = doc(this.firestore, `${this.auth.resource.env.db.users}`, `${UID}`)
      return getDoc(userRef)
    }

    getUserList(c:number){
      const catData:CollectionReference = collection(this.firestore, `${this.auth.resource.env.db.users}`)
    ...
    getStoreList(c:number){
      const catData:CollectionReference = collection(this.firestore, `${this.auth.resource.env.db.shops}`)
      const qu = query(catData, 
        //where("ban", "==", false),
        orderBy("sin", "desc"),
        limit(c)
    ...
    getProductByID(ID:string){
      const userRef = doc(this.firestore, `${this.auth.resource.env.db.things}`, `${ID}`)
      return getDoc(userRef)
    }

    getBurnProductList( c:number ){
      const catData:CollectionReference = collection(this.firestore, `${this.auth.resource.env.db.things}`)
      const qu = query(catData, 
        where("burn", "==", true),
        //where("ban", "==", false),
        //where("sid", "==", sid),
    ...
    reqBurn(id:string){
      const productRef = doc(this.firestore, `${this.auth.resource.env.db.things}`, `${id}`);
    ...
    deleteProduct(storeID:string, id:string){
      const productRef = doc(this.firestore, `${this.auth.resource.env.db.things}`, `${id}`);
      const storeRef = doc(this.firestore, `${this.auth.resource.env.db.shops}`, `${storeID}`);
    ...
    getCampaignList(sid:string //, c:number
      ){
      const catData:CollectionReference = collection(this.firestore, `${this.auth.resource.env.db.hypes}`)
      const qu = query(catData, 
        //where("ban", "==", false),
        where("sid", "==", sid),
        orderBy("sin", "asc"),
    ...
    addNewCampaign(tX:string, 
      ...
      const hypeRefC = collection(this.firestore,`${this.auth.resource.env.db.hypes}`);
      return addDoc(hypeRefC, dataSend).then(ref => {
        console.log("DONE", ref.id)
    
        const cB = ( data.type == "flat" ? data.cbNew: data.max);
        const userRef = doc(this.firestore, `${this.auth.resource.env.db.users}`, `${data.by}`);
        const shopRef = doc(this.firestore, `${this.auth.resource.env.db.shops}`, `${data.sid}`);
    
        return updateDoc(userRef, {storeCam: arrayUnion(ref.id), cashback: cB}).then(() => {
          return updateDoc(shopRef, { cashback: cB }).then(() => {
          console.log("DONE2")
    
          const hypeRef = doc(this.firestore, `${this.auth.resource.env.db.hypes}`, `${ref.id}`);
    ...
    async updateStoreBio(
      ...
      //   let looks:string[] = this.resources.getLOOKUP( username +' '+ name.toLowerCase() );
         const userRef = doc(this.firestore, `${this.auth.resource.env.db.shops}`, `${id}`);
         return updateDoc(userRef, { 
           name:name, 
           //soIG, soYT, soTW, soWA,
           //info:info, url:url, typ:typ, sex:sex, stat:stat, look:looks, 
    ...
    async removeProductBurnBanners(id:string, banner:string){
      const newTimestamp = this.getServerTimestamp();
      const userRef = doc(this.firestore, `${this.auth.resource.env.db.things}`, `${id}`);
      return updateDoc(userRef, { burnPics: arrayRemove(banner), upd: newTimestamp });
    }
    ...
    async addProductBurnBanners(id:string, banner:string){
      ...
      if(!cloudUpload.success){
        ...
        return updateDoc(userRef, { burnPics: arrayUnion(cloudUpload.url), upd: newTimestamp }).then(() => {
          return cloudUpload;
        });
      }
    }
    ...
    addProduct(data:any, banners:string[]){
      ...
      const thingsRefC = collection(this.firestore, `${this.auth.resource.env.db.things}`)
      return addDoc(thingsRefC, dataSend).then(ref => {
        const storeRef = doc(this.firestore, `${this.auth.resource.env.db.shops}`, `${data.storeID}`);
        return updateDoc(storeRef, {
          products: increment(1),
          proCat: arrayUnion(data.category),
        }).then(async () => {
          const thingsRef = doc(this.firestore, `${this.auth.resource.env.db.things}`, `${ref.id}`);
...
async updateProduct(id:string, data:any ){
  ...
  const thingsRef = doc(this.firestore, `${this.auth.resource.env.db.things}`, `${id}`)
...
async updateProductBurn(id:string, data:any ){
  ...
  const thingsRef = doc(this.firestore, `${this.auth.resource.env.db.things}`, `${id}`)
  return updateDoc(thingsRef, {
    reqBurn: false, burn: true, flash: data.flash,

    burnPic:data.burnPic, burnPics:data.burnPics,
    burnCat:data.burnCat, burnCatSub:data.burnCatSub,
    costBurn:data.costBurn,
    burnBrand:data.burnBrand, burnDecShort:data.burnDecShort,
    burnSpec:data.burnSpec, burnDec:data.burnDec,
...
async createBurnCat(id:string, tit:string, img:string){
  ...
  const burnCatRef = doc(this.firestore, `${this.auth.resource.env.db.burnCats}`, `${id}`)
