...
export class UploadProductComponent implements OnInit {
  ...
  onFileChange(evt: any) {
    ...
    if(!uid || !emailORphone || !sid || !cat){
      ...
      !storeProduct.code //|| !storeProduct.warranty || !storeProduct.content
          ){
                    ...
                    if(storeProduct.price < storeProduct.cost){
                      ...
                      if(!storeProduct.category){
                        ...
                        if(!storeProduct.code){
                          ...
                          //if(!storeProduct.content || !storeProduct.warranty){
                            console.log("Product content/warranty is required");
                          //}else{
                            console.log("Invalid Fields");
                          //}
                        }
  ...
  setUpFile(uid:string){
    ...
    if(!this.storeData.sid){
      ...
      this.master.getStoreByID(this.storeData.sid).then(sX => {
        const storeINFO:any = sX.exists() ? sX.data() : null;
        ...
        if(!storeINFO){
          ...
          if(uid !== storeINFO.by){
            ...
            const storeX = {
              ...
              email:storeINFO.email, phone:storeINFO.phone, name: storeINFO.name
            }
            console.log(storeX)
            this.export(storeX.uid, storeX.sid, storeX.cat, storeX.email, storeX.phone, storeX.name)
          }
  ...
  export(uid: string, sid: string, cat: string, email: string, phone: string, storeName:string): void {    
    const sample: AOA = [
    [
        null,
        "uid",
    ...
    const fileName: string = 'Refr-Product-Sheet-' + storeName + '.xlsx';
    /* save to file */
