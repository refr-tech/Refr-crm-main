...
export class PayComponent implements OnInit {
  ...
  constructor(
    ...
    if(!data){
      ...
      if(this.paymentData.from == 'CAMP_ADMIN'){
        console.log("paymentData", this.paymentData)
      this.pay.startAdminGatewayCamp(this.data).then(ref => {
        if(!ref){
          this.payFailed("Payment Failed, Try again...");
        }else{
          this.paymentId = ref.id;
      ...
      const amtX = this.paymentData.amGive;

        this.pay.addVendorHypeBalanceADMIN(
          this.paymentId, this.paymentData.by, amtX, this.paymentData.admin, this.paymentData.name
          //,this.paymentData.amTotal, gwID,gwSIGN,gwORDR, gwInfo
        ).then(() => {
          this.pay.records("vendor", this.paymentData.amGive)
          this.pay.records("refr", this.paymentData.amPaid)
          setTimeout(() => {
      ...
      if(this.paymentData.from == 'RCASH_ADMIN'){
        console.log("paymentData", this.paymentData)
      this.pay.startAdminGatewayRefrCASH(this.data).then(ref => {
        if(!ref){
          this.payFailed("Payment Failed, Try again...");
        }else{
          this.paymentId = ref.id;
      ...
      const amtX = this.paymentData.amGive;
        ...
        this.pay.addClientBalanceADMIN(
          this.paymentId, this.paymentData.by, amtX, this.paymentData.admin, this.paymentData.name
          //,this.paymentData.amTotal, gwID,gwSIGN,gwORDR, gwInfo
        ).then(() => {
          this.pay.records("client", this.paymentData.amGive)
          this.pay.records("refr", this.paymentData.amPaid)
